    ...
    <%
        ...
        if (request.getParameter("kies_knop") != null) {
            ...
            if (!request.getParameter("min_prijs").equals("")) {
                minPrijs = Integer.parseInt(request
                        .getParameter("min_prijs"));
            }
            if (!request.getParameter("max_prijs").equals("")) {
                maxPrijs = Integer.parseInt(request
                        .getParameter("max_prijs"));
            }
            for (Auto a : autos) {
                if (merk.equals("alle merken")) {
                    if (a.getPrijs() <= maxPrijs
                            && a.getPrijs() >= minPrijs) {
                        selectie.add(a);
                    }
                } else {
    ...
    <div class="col-md-9">
        ...
        <div class="bs-callout bs-callout-warning">
            ...
            <form action="demo.jsp" method="get">
                <label>Kies Merk</label> <select name="merk">
                <option value="alle merken">alle merken</option>
                <%
                    for (String merk : merken) {
                        out.println("<option value=\"" + merk + "\"></option>");
                    }
                %>

            </select><br> <label>Minimum prijs</label> <input type="number"
                                                              name="min_prijs"> <label>Maximum prijs</label> <input
                    type="number" name="max_prijs"><br> <input
                    type="submit" name="kies_knop" value="Ok">
