...
public class SportServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		if (req.getParameter("nieuwlidform") != null) {
			...
			RequestDispatcher disp = req.getRequestDispatcher("/AO/JSP_Java_DB/deel4JQV2.jsp");
			disp.forward(req, resp);
		}
		else {
			req.setAttribute("via_servlet", "true");
			RequestDispatcher disp = req.getRequestDispatcher("/AO/JSP_Java_DB/deel4JQV3.jsp");
			disp.forward(req, resp);
		}
	}
	...
	private void voegNieuwLidToe(HttpServletRequest req) {
         Lid lid = new Lid(req.getParameter("roepnaam"), req.getParameter("tussenvoegsels"), 
        		 req.getParameter("achternaam"), req.getParameter("adres"), 
        		 req.getParameter("postcode"), req.getParameter("woonplaats"), req.getParameter("telefoon"), 
        		 req.getParameter("email"), req.getParameter("geboortedatum"), req.getParameter("geslacht"));
	...
	private void voegTeamspelerToe(HttpServletRequest req, HttpServletResponse resp) 
		...
		resp.sendRedirect("/sporthtml?haal_team_gegevens=x&teamcode=" + team.getTeamcode() );
	}
	
	private void verwijderTeamspeler(HttpServletRequest req, HttpServletResponse resp) 
			throws IOException, ServletException {
		Lid lid = null;
		admin = new Administratie();
