...
public class AutoshopServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		if (req.getParameter("merk") == null) {
			...
			try {
				...
				if (req.getParameter("merk") != null && !req.getParameter("merk").equals("")) {
					merk = req.getParameter("merk");
				}
				if (!req.getParameter("minprijs").equals("")) {
					try {
						minPrijs = Integer.parseInt(req.getParameter("minprijs"));
					} catch (NumberFormatException | NullPointerException e) {}
				}
				if (!req.getParameter("maxprijs").equals("")) {
					try {
						maxPrijs = Integer.parseInt(req.getParameter("maxprijs"));
					} catch (NumberFormatException | NullPointerException  e) {}
				}
				...
				if (req.getParameter("merk") != null) {
					...
					if (merk.equals("alle")) {
						...
						for (Auto auto:lijst) {
							if (auto.getPrijs() > minPrijs && auto.getPrijs() < maxPrijs && auto.getMerk().equals(merk)) {
								out.println(maakAutoDiv(auto));
							}
						}
					}
	...
	private String maakAutoDiv(Auto auto) {
		...
		div += "<img src=\"" + auto.getFoto() + "\" alt=\"" + auto.getMerk() + " " + auto.getType() + "\">\n";
		div += "<p class=\"merktype\">" + auto.getMerk() + " " +  auto.getType() + "</p>\n";
