...
<%	if (user != null) { %>
      ...
      <h2>2 Fooi Calculator</h2>
        ...
        <div class="bs-callout bs-callout-warning">
			...
			<ul>
				<li>een scherm kunt maken met TextViews, EditText velden en Buttons</li>
				<li>weet hoe en waarom je elementen een id geeft</li>
				<li>weet hoe je width, height, margin, padding en TextSize gebruikt</li>
				<li>weet hoe je verschillende types invoervelden (EditText) maakt</li>
				<li>weet hoe je views in je java klassen kunt aanroepen</li>
				<li>weet hoe je tekst uit een invoerveld kunt aanroepen</li>
				<li>weet hoe je een OnClickListener en een OnEditorActionListener gebruikt</li>
				<li>weet hoe je waardes kunt bewaren als je app onPause() gaat en weer inlaadt 
				als hij wordt hervat</li>
			</ul>
		...
		<p>In dit hoofdstuk gaan we een app maken die op basis van een rekeningbedrag en een fooipercentage 
					...
					<p>De default layout die studio aanmaakt bij een nieuw project is <em>&quot;RelativeLayout&quot;</em>. 
					Deze layout positioneert de elementen op het scherm relatief ten opzichte van elkaar.</p>
					...
					de layout schermvullend is. De widgets krijgen standaard de waarde &quot;wrap_content&quot; Dit betekent dat de elementen precies 
					groot genoeg worden om hun inhoud te bevatten. Meestal kun je dit zo laten.</p>
					
					<h3>margin, padding en textSize</h3>
					<p>Margin en padding worden uitgedrukt in <b>dp</b> (density-independant-pixels). TextSize in <b>sp</b> (scale-independant-pixels). 
					<p>Afmetingen (dimensies) kunnen worden opgeslagen in het bestand values/dimens.xml. Reden om dimensies in aparte bestanden 
					op te slaan is dat je op die manier verschillende files voor verschillende devices (telefoon, tablet, tv etc) kunt maken.</p>
					
					<h3>EditText</h3>
					<p>Met een EditText element kan gebruiker tekst invoeren. De grootte van het invoerveld kan worden aangegeven met de property ems 
					(em is de breedte van de letter m). De property inputType bepaalt o.m. wat voor toetsenbord er verschijnt. Voor de fooi calculator 
					kun je het best Number (Decimal) gebruiken.
<pre class="code">
&lt;EditText
...
&lt;/EditText&gt;
					...
					<div class="opdrachten">
						<h2>Opdracht 2.1</h2>
						<p>Maak de User Interface van de fooi calculator app met een Relative Layout. Maak eerst de strings aan in strings.xml. Experimenteer 
						met de elementen in de Design Tab en bekijk de properties in de Text tab die studio heeft aangemaakt.</p>
					</div>
					<div class="toonknopcontainer">
							<button class="toonknop" id="toon2_1" onclick="toon('uitwerking2_1')">Toon uitwerking</button>
					</div>
					...
					<p>Studio heeft een java klasse aangemaakt die je kunt vinden in de map java/[packagenaam]. De klasse is een uitbreiding 
					(extensie) van ActionBarActivity. Omdat we nog geen Action Bar gebruiken kun je dit veranderen in Activity.</p>
<pre class="code">
public class FooiCalculatorActivity extends <span class="codeplus">Activity</span> {
</pre>					
					
					<p>In de klasse hebben we de volgende elementen uit de user interface nodig:</p>
					<ul>
						<li>het EditText veld om gebruikersinvoer op te halen</li>
						<li>de TextView waarin het percentage wordt getoond en waarvan de tekst veranderd wordt als gebruiker
						op &eacute;&eacute;n van de knoppen drukt</li>
...
public class FooiCalculatorActivity extends Activity {
    ...
    private double fooiPercentage = 15.0;
					...
					<p>Als een app wordt gestart wordt de methode <b>onCreate()</b> aangeroepen. In deze methode gaan we de verschillende views 
					initialiseren. Dit doen we met de methode 
					<a href="http://developer.android.com/reference/android/view/View.html#findViewById(int)" target="_blank">&quot;findViewById&quot;</a>. 
					met als argument R.id.[naam id]. Deze methode geeft een object van het type View terug. Door te typecasten geef je aan om wat voor 
					soort type View het gaat. De id komt overeen met de id die je in de xml file aan het element hebt gegeven. Merk op dat 
					het nu handig is als de naam van je id &quot;betekenisvol&quot; is en het type View bevat.</p>
...
protected void onCreate(Bundle savedInstanceState) {
  ...
  totaalTextView = <span class="codeplus">(TextView) findViewById(R.id.totaalTextView);</span>
					...
					<p>De bedoeling van de app is dat als gebruiker een bedrag invoert en bevestigt, het fooibedrag en totaalbedrag 
					worden vertoond. Als gebruiker op &eacute;&eacute;n van de knoppen drukt, moet het percentage veranderen en de berekening 
					worden aangepast. Om niet steeds dezelfde code te hoeven schrijven maken we een hulpmethode <b>berekenEnToon()</b> die we 
					straks gaan gebruiken in de action listeners.</p>
<pre class="code">
private void berekenEnToon() {
        ...
        <span class="codeplus">procentTextView.setText(&quot;&quot; + (int) fooiPercentage + &quot;%&quot;);</span>
					...
					<ul>
						<li>een listener van het type <a href="http://developer.android.com/reference/android/widget/TextView.OnEditorActionListener.html" target="_blank">OnEditorActionListener</a> voor het invoerveld</li>
						<li>een listener van het type <a href="http://developer.android.com/reference/android/view/View.OnClickListener.html" target="_blank">OnClickListener</a> voor de knoppen</li>
					</ul>
					<p>Om de listeners te maken kunnen we de twee interfaces implementeren in onze klasse. Voeg de volgende regels toe aan je 
...
public class FooiCalculatorActivity extends Activity
        <span class="codeplus">implements OnEditorActionListener, OnClickListener</span> {
					...
					<p>De implementatie van deze &quot;interfaces&quot; vereist dat je twee methoden implementeert:</p>
					<ul>
						<li>de methode <b>onEditorAction()</b> van de OnEditorActionListener interface</li>
						<li>de methode <b>onClick()</b> van de OnClickListener interface</li>
					</ul>
					<p>In deze methodes kun je de hulpmethode <b>berekenEnToon()</b> aanroepen om de berekening te maken en 
					de scherm elementen aan te passen.</p>
					<p>In de methode <b>onEditorAction()</b> moet eerst worden bepaald wat de gebruiker heeft gedaan. Hiervoor kun je 
					constanten van de klasse <a href="http://developer.android.com/reference/android/view/inputmethod/EditorInfo.html" 
					target="_blank">EditorInfo</a> gebruiken.</p>
					<p>In de methode <b>onClick()</b> moet eerst het id van de knop worden opgevraagd om te bepalen wat er moet gebeuren.</p>
<pre class="code">
@Override
    ...
    switch (v.getId()) {
        ...
        case R.id.procentPlusButton:
            ...
            break;
					...
					<p>Nu kunnen we de listeners aan de views koppelen in de <b>onCreate()</b> methode. Omdat onze klasse de listener 
					interfaces implementeert kan dit met het keyword <b>this</b> worden gedaan.</p>
<pre class="code">
protected void onCreate(Bundle savedInstanceState) {
    super.onCreate(savedInstanceState);
    setContentView(R.layout.activity_fooi_calculator);
    ...
    <span class="codeplus">procentPlusButton.setOnClickListener(<b>this</b>);</span>
					...
					<div class="opdrachten">
						<h2>Opdracht 2.2</h2>
						<p>Verander met bovenstaande aanwijzingen de java klasse en test de app.</p>
					</div>
					
					<div class="opdrachten">
						<h2>Opdracht 2.3</h2>
						<p>Maak een app waar gebruiker een bedrag kan invoeren. Afhankelijk van het bedrag wordt de 
						korting berekend:</p>
						<ul>
							<li>bedrag &lt; 100: 0%</li>
				...
				fooipercentage.</p>
				<p>Declareer een object van het type SharedPreferences en zorg er voor dat de waardes die je wilt bewaren globaal gedeclareerd 
				zijn.</p>
...
public class MainActivity extends Activity
    ...
    ...
				...
				<p>Het SharedPreferences object instantieer je in de methode <b>onCreate()</b> op de volgende wijze:</p>
				
<pre class="code">
bewaardeWaardes = getSharedPreferences("BewaardeWaardes", MODE_PRIVATE);
</pre>									
				<p>Het eerste argument is de naam van het bestand dat wordt gemaakt en het tweede argument maakt dat het 
				bestand alleen voor de eigen applicatie beschikbaar is.</p>
				<p>We willen dat de waardes opgeslagen worden als de app onPause() aanroept om op de pauzestand te gaan. Dus 
				moeten we deze methode van de superklasse &quot;overriden&quot;. In deze methode hebben we een 
				<a href="http://developer.android.com/reference/android/content/SharedPreferences.Editor.html" target="_blank">Editor</a> nodig die 
				de waardes van de variabelen aan het bestand kan toevoegen. let op, je kunt de meeste primitieve waardes bewaren, maar geen doubles!</p>
<pre class="code">
@Override
public void onPause() {
    ...
    super.onPause();
				...
				<p>Als de app weer wordt hervat, wordt de methode onResume() aangeroepen, dus die gaan we ook overriden. Daarbij 
				moeten de bewaarde gegevens worden geladen.</p>
...
public void onResume() {
    ...
    berekenEnToon();
				...
				<div class="opdrachten">
					<h2>Opdracht 2.4</h2>
					<p>Zorg er voor dat je fooi calculator waardes opslaat als hij op pauze gaat en weer laadt als hij wordt hervat.</p>
				</div>
