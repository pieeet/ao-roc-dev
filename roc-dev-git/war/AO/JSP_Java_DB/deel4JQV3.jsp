...
<div class="col-md-9">
	...
	<div class="bs-callout bs-callout-warning">
		...
		<ul>
			<li>met behulp van Java, JSP, CSS, een database en eventueel
				JavaScript een web applicatie met een database kunt ontwikkelen</li>
			...
			<div id="tabs_sport">
				...
				<div id="tabs_sport-1" class="sport_content">
					...
					<div id="sport_tab-1_content" class="sport_content">
						<img id="loading_spinner"
							src="/AO/JSP_Java_DB/images/loading_spinner.gif">
					</div>
				</div>
				<div id="tabs_sport-2">
					<img src="/AO/JSP_Java_DB/images/sportLogo.png" id="sport_logo">
	...
	<p>
		...
		database werken, dan kun je <a href="sport?sql=true">hier</a> terecht.
	</p>


	<p>Voor deze opdracht gebruik je:
	<ul>
		<li>een geschikte IDE. We gaan er van uit dat je Eclipse met de <a
			href="https://cloud.google.com/appengine/docs/java/tools/eclipse#Installing_the_Google_Plugin_for_Eclipse"
			target="_blank">App Engine plugin</a> gebruikt, maar er is ook een
			goed functionerende (niet offici&euml;le) <a
			href="https://code.google.com/p/nb-gaelyk-plugin/downloads/list"
			target="_blank">plugin</a> voor NetBeans beschikbaar, mocht je daar
			liever mee werken.
		</li>
	</ul>
	...
	<p>
		...
		veelgebruikte <a href="#"> Model-View-Controller</a> ontwerp patroon..
		Maak in de map &quot;src&quot; 3 packages, volgens onderstaand
		voorbeeld.
	</p>
	<img src="/AO/JSP_Java_DB/images/project_mvc.png">
	<h3>Model laag</h3>
	<p>De &quot;model&quot; laag bestaat uit klassen die een
		afspiegeling zijn van iets uit de werkelijkheid. In ons project hebben
		we te maken met een vereniging met spelers en teams.</p>
	<img src="/AO/JSP_Java_DB/images/model_klassen.png">
	<p>Maak de klasse &quot;Lid&quot;, geef hem een aantal attributen
		zoals roepnaam, tussenvoegsels en achternaam. Ook hebben we een unieke
		sleutel nodig om het lid straks gemakkelijk te kunnen identificeren.
		Hiervoor zou je bijvoorbeeld een e-mail adres kunnen gebruiken,
		aangezien deze altijd uniek is. Tot slot maken we een standaard
		constructor, get en set methoden (getters en setters) en een methode
		die de gehele naam van het lid teruggeeft.</p>
...
public class Lid {
	...
	<p>Het eerste wat onze app moet kunnen is nieuwe leden aanmaken, ze
		opslaan in de database en ze daaruit weer ophalen. Maak een jsp pagina
		(bijvoorbeeld index.jsp) met een formulier en de benodigde
		invoervelden met gegevens waarmee een nieuw lid kan worden aangemaakt.</p>
	<p>
	...
	<h3>Servlets</h3>
	<p>
		Als je een html formulier naar de server stuurt moet die ergens worden
		opgevangen en verwerkt. Hiervoor gebruiken we een <a
			href="http://docs.oracle.com/javaee/7/api/javax/servlet/http/HttpServlet.html"
			target="_blank">Servlet</a>.
	</p>

	<p>Maak in je package &quot;control&quot; een java-klasse
...
public class SportServlet extends <span class="codeplus">HttpServlet</span> {
	...
	<p>Klik op de foutmeldingen die Eclipse geeft en importeer de
		benodigde klassen uit de &quot;javax.servlet&quot; packages</p>
	<p>In de servlet gaan we twee methoden uit de superklasse
		&quot;overriden&quot;. Klik in Eclipse op het menu-item
		&quot;Source&quot; en kies de optie &quot;Override/Implement
		methods...&quot; Zorg dat de methoden doGet en doPost worden
		ge&iuml;mplementeerd in je klasse. Zie voorbeeld.</p>
	<pre class="code">
    @Override
	...
	<p>Om het jezelf gemakkelijk te maken hoef je maar
		&eacute;&eacute;n van de twee methoden uit te werken en de ander naar
		deze methode door te sturen. Je hoeft je dan geen zorgen meer te maken
		over wanneer get en post worden gebruikt.</p>
	<p>Om requests naar een servlet te sturen heeft de servlet een
		adres (url) nodig. Je kunt de url van een servlet &quot;mappen&quot;
		in het web-xml bestand dat zich in de map WEB-INF van je war directory
		bevindt.</p>
	<pre class="code">
&lt;servlet&gt;
...
&lt;/form&gt;
	...
	<p>In je servlet gebruik je de meegestuurde parameters om een
		...
		daadwerkelijk op de knop is geklikt, vervolgens vraag je de parameters
		op en maak je een lid aan.</p>
...
public void doGet(HttpServletRequest req, HttpServletResponse resp) 
	...
	<h3>Datastore</h3>
	<p>Het lid-object willen we opslaan in de database. In je package
		sportIO maak je een klasse die verantwoordelijk wordt voor alle
		...
		Entities. Als je een entity aanmaakt geef je de naam van de tabel (met
		hoofdletter) waarin hij wordt opgeslagen en een unieke sleutel waarmee
		je hem kunt identificeren. In ons geval gebruiken we daarvoor het
		attribuut spelerscode. Nadat je de entity hebt aangemaakt kun je hem
		properties meegeven. Dit zijn de attributen van je lid-object die
...
public class SportIO {
	...
	<p>In je servlet maak je een object van de SportIO klasse en
		gebruikt de methode die we net hebben gemaakt om het lid op te slaan.
...
public void doGet(HttpServletRequest req, HttpServletResponse resp) 
	...
	<p>De volgende stap is om je opgeslagen leden uit datastore op te
		vragen en op een jsp pagina te vertonen. Hiervoor moet je in SportIO
...
public void doGet(HttpServletRequest req, HttpServletResponse resp) 
    ...
    if (req.getParameter(&quot;verzend_nieuw_lid_knop&quot;) != null) {
    	...
    	<span class="codeplus">ArrayList&lt;Lid&gt; leden = <span
			class="jsp">io.getAlleLeden()</span>;</span>
	...
	<p>
		...
		opgevraagd en als attribuut aan het req object gekoppeld met de
		methode <em>setAttribute()</em>. De servlet stuurt vervolgens een jsp
		pagina met het attribuut &quot;leden&quot; terug naar de browser van
		de gebruiker met behulp van een RequestDispatcher object. In de jsp
		pagina kun je het attribuut nu gebruiken (zie code). Let op dat je de
		nodige java klassen in de pagina importeert.
	</p>
...
<span class="jsp">&lt;%</span>  }
	...
	<p>Om een enkele entity uit de datastore te halen kun je z&#39;n
		key gebruiken, in ons geval de spelerscode. Eclipse zal aangeven dat
		je een EntityNotFoundException moet afvangen (try/catch block) of
		doorgeven (throws declaratie) aan de klasse die de methode aanroept.
		In onderstaande uitwerking is er voor gekozen om de Exception af te
		vangen. Mocht de entity, om wat voor reden dan ook, niet (meer)
...
public void doGet(HttpServletRequest req, HttpServletResponse resp) 
	...
	<p>In bovenstaande code wordt een pagina genaamd lid.jsp naar
		...
		gegevens van het lid vertonen en twee knoppen met toepasselijke namen.
		Om deze pagina te kunnen maken heeft hij het attribuut &quot;lid&quot;
		nodig. Als dit attribuut ontbreekt (== null) kun je de gebruiker terug
		sturen naar het ledenoverzicht.</p>
...
public void wijzigLid(Lid lid) {
	...
	<p>Maak deze methodes aan in je SportIO klasse. Gebruik de methodes
		in je servlet zodat een lid wordt gewijzigd of verwijderd als
		gebruiker op &eacute;&eacute;n van de knoppen klikt die we aan het
		formulier hebben toegevoegd. Nadat een lid is gewijzigd of verwijderd
		kun je de bezoeker terug sturen naar het ledenoverzicht.</p>
	<div class="bs-callout bs-callout-warning">
		<h3>Test</h3>
	...
	<p>
		Tot nu toe hebben we alleen strings opgeslagen, maar je kunt uiteraard
		ook andere types opslaan. Zie <a
	...
	<p>Om alleen entities met een bepaalde property te selecteren kun
		...
		een bepaalde speler. Om dit mogelijk te maken maak je in datastore een
		tabel &quot;Teamspeler&quot;.</p>
